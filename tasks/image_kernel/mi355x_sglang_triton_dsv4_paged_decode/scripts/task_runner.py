...
Target device kernels : ``_paged_decode_split_kernel`` + ``_paged_decode_reduce_kernel``
Timed launcher        : ``sparse_attn_v4_paged_decode``
Source                : sglang/kernels/ops/attention/dsv4/unified_kv_kernels/paged_decode.py

This is the decode attention SGLang actually ships on MI355X: AMD's nightly CI pins
``SGLANG_HACK_FLASHMLA_BACKEND=unified_kv_triton``, which routes here.
...
def _make(case: dict, correctness: bool = False) -> dict:
    ...
    p = case["params"]
    T, H, D = p["t"], p["h"], p["d"]
    kv_len, page_size = p["kv_len"], p["page_size"]
    ...
    n_pages = max(kv_len * 4, 4096)
    unified_kv = torch.randn(n_pages, D, device="cuda", dtype=torch.bfloat16)
    q = torch.randn(T, H, D, device="cuda", dtype=torch.bfloat16)
    # Ragged CSR over the unified pool: one contiguous run of kv_len pages per query.
    idx = torch.randperm(n_pages, device="cuda")[: T * kv_len].to(torch.int32)
    kv_indices = idx.contiguous()
    kv_indptr = torch.arange(0, (T + 1) * kv_len, kv_len, device="cuda", dtype=torch.int32)
...
def _reference(inputs: dict):
    """Dense fp32 attention over the gathered pages, with the zero-valued sink.

    out_t = softmax([scale * q_t . kv_j for j in pages(t)] ++ [sink_h]) @ ([kv_j] ++ [0])
    """
    torch = _torch()
    q = inputs["q"].float()
    kv = inputs["unified_kv"].float()
    ...
    for t in range(T):
        sel = kv.index_select(0, indices[indptr[t] : indptr[t + 1]].long())  # [K, D]
        logits = (q[t] @ sel.T) * scale                                      # [H, K]
        m = torch.maximum(logits.max(dim=-1).values, sink)                   # [H]
        p = torch.exp(logits - m[:, None])
        denom = p.sum(dim=-1) + torch.exp(sink - m)                          # sink value is 0
        out[t] = (p @ sel) / denom[:, None]
...
def _perturb_inputs(inputs: dict) -> None:
    """Refresh q and the KV pool in place with values no earlier launch has seen.

    A replayed graph reads the captured addresses, so writing through them changes what
    the scored kernel consumes. Indices and indptr are left alone: the ragged structure
    is what the case is defined by, and changing it would change the test.
    """
    torch = _torch()
...
def run_compile() -> None:
    ...
    print("dsv4_paged_decode compile smoke: PASS")
